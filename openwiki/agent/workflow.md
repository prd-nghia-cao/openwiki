...
- **baseten / fireworks / openai-compatible**: `new ChatOpenAI({ apiKey, configuration: { baseURL?, defaultHeaders?, defaultQuery? }, model })` — OpenAI-compatible clients using the provider's base URL when configured. The `openai-compatible` provider has no default endpoint; its base URL is user-supplied via `OPENAI_COMPATIBLE_BASE_URL` and required (`requiresBaseUrl: true`), which lets OpenWiki target any OpenAI-compatible gateway (for example a LiteLLM gateway fronting upstream providers). When `OPENAI_COMPATIBLE_HEADERS` is set (a JSON object of string values), those headers are passed through as `configuration.defaultHeaders` on the ChatOpenAI client; `resolveProviderHeaders()` validates the value at startup and aborts the run early on malformed input. When `OPENAI_COMPATIBLE_QUERY` is set (a raw query string), its parsed parameters are passed through as `configuration.defaultQuery` and appended to every request; `resolveProviderQuery()` parses it with `URLSearchParams` (duplicate keys last-wins).
- **workday-cis**: `new ChatWorkdayCis({ model, baseURL, apiKey?, headers?, query?, targetProvider, taskType, predictionType?, generationConfig })` — talks to Workday CIS `POST /v1alpha1/predictions/stream` (Gemini-shaped request/response over Server-Sent Events) with full function/tool calling support. The provider has no default endpoint; `WORKDAY_CIS_BASE_URL` is required (`requiresBaseUrl: true`). `WORKDAY_CIS_TARGET_PROVIDER` is required (`target.provider` in the CIS envelope); `target.model` comes from `OPENWIKI_MODEL_ID`. Optional `WORKDAY_CIS_API_KEY` is sent as a bearer token. `WORKDAY_CIS_HEADERS` and `WORKDAY_CIS_QUERY` follow the same validation and passthrough rules as the openai-compatible provider (`resolveProviderHeaders()`, `resolveProviderQuery()`). `WORKDAY_CIS_TASK_TYPE` defaults to `gcp-multimodal-v2`; `WORKDAY_CIS_PREDICTION_TYPE` is omitted when unset; `WORKDAY_CIS_GENERATION_CONFIG` merges over defaults `{temperature:0.2,maxOutputTokens:8192,topK:40,topP:0.95}`.
